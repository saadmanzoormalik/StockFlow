...
const actionStyles: Record<DailyIntelligenceItem["action"], string> = {
...
export function DailyIntelligenceFeed({ items }: { items: DailyIntelligenceItem[] }) {
  return (
    <section className="space-y-4">
      <div>
        <p className="text-sm font-semibold text-signal dark:text-sky-300">5-minute daily edge</p>
        <h2 className="mt-1 text-2xl font-bold leading-tight">Today&apos;s Intelligence Feed</h2>
        <p className="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-300">
          A short daily layer for action, knowledge, risk, and personalization signals.
        </p>
      </div>
